...
vector<int> gr[MAXN];
int dfn[MAXN << 1], cnt, w[MAXN << 1], a[MAXN], in[MAXN], out[MAXN];
...
inline void work(signed CASE = 1, bool FINAL_CASE = false) {
  ...
  dfs(rt, 0);
  for (int i = 1; i <= cnt; i++) add(i, w[i]);
  for (int op, x, d, i = 1; i <= m; i++) {
    cin >> op;
    if (op == 1) {
      cin >> x >> d;
      add(in[x], d);
      ...
      cin >> x;
      cout << ((query(out[x]) - query(in[x] - 1)) >> 1) << endl;
    }
  }
}
