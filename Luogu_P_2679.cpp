...
inline void work(signed CASE = 1, bool FINAL_CASE = false) {
  ...
  scanf("%s", b + 1);
  dp[0][0][0][0] = 1;
  for (int now = 1, i = 1; i <= n; i++, now = i & 1) {
    dp[now][0][0][0] = 1;
    for (int j = 1; j <= min(i, m); j++) {
      for (int k = 1; k <= min(j, ek); k++) {
        dp[now][j][k][0] = dp[now][j][k][1] = 0;
        dp[now][j][k][0] = (dp[now ^ 1][j][k][0] + dp[now ^ 1][j][k][1]) % mod;
        if (a[i] == b[j])
          dp[now][j][k][1] =
              ((dp[now ^ 1][j - 1][k][1] + dp[now ^ 1][j - 1][k - 1][0]) % mod +
               dp[now ^ 1][j - 1][k - 1][1]) %
              mod;
      }
    }
  }
  cout << (dp[n & 1][m][ek][0] + dp[n & 1][m][ek][1]) % mod;
}
