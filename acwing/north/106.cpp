...
inline void work(signed CASE = 1, bool FINAL_CASE = false) {
  ...
  cout << c << " " << (n + 1) / 2 << endl;
  int cnt = 0;
  priority_queue<int, std::vector<int>> g;
  priority_queue<int, std::vector<int>, greater<int>> l;
  for (int i = 1; i <= n; i++) {
    int x;
    cin >> x;
    if (g.empty() || g.top() > x) {
      ...
      l.push(x);
    while (g.size() > l.size() + 1) l.push(g.top()), g.pop();
    while (l.size() > g.size()) g.push(l.top()), l.pop();
    if (i & 1) {
      cnt++;
      cout << g.top() << " ";
      if (cnt % 10 == 0) puts("");
