...
int h[MAXN], idx, nex[MAXN], e[MAXN], in[MAXN], out[MAXN], dp[MAXN];
const int M = 80112002;
inline void add(int u, int v) {
  e[idx] = v, nex[idx] = h[u], h[u] = idx++, in[v]++, out[u]++;
}
inline void work(signed CASE = 1, bool FINAL_CASE = false) {
  ...
  while (!q.empty()) {
    ...
    for (int i = h[now]; ~i; i = nex[i]) {
      int v = e[i];
      dp[v] = (1ll * dp[now] + dp[v]) % M;
      if (!(--in[v])) q.push(v);
    }
  }
  int ans = 0;
  for (int i = 1; i <= n; i++) {
    if (!out[i]) ans = (ans + dp[i] * 1ll) % M;
  }
  cout << ans;
}
