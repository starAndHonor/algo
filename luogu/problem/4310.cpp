...
int n, a[MAXN], dp[MAXN], bits[33];
map<int, int> lo;
inline int lowbit(int x) { return x & (-x); }
int ans;
inline void work(signed CASE = 1, bool FINAL_CASE = false) {
  cin >> n;
  for (int i = 1; i <= 33; i++) lo[{1 << i}] = i;
  for (int i = 1; i <= n; i++) {
    cin >> a[i];
  }
  for (int i = 1; i <= n; i++) {
    int x = a[i];
    while (x) dp[i] = max(bits[lo[(lowbit(x))]] + 1, dp[i]), x -= lowbit(x);
    x = a[i];
    while (x)
      bits[lo[(lowbit(x))]] = max(dp[i], bits[lo[(lowbit(x))]]), x -= lowbit(x);
    ans = max(ans, dp[i]);
  }
  cout << ans;
}
