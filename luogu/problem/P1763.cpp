...
int ans[MAXN];
int tem[MAXN];
inline void easify(int &a, int &b) {
  int c = __gcd(a, b);
  a /= c;
  b /= c;
}
int maxx = -INF;
bool dfs(int d, int a, int b, int last) {
  easify(a, b);
  if (d == dep + 1) {
    if (!a) {
      if (ans[dep] > tem[dep]) {
  ...
  if (b * (dep + 1 - d) > maxx * a || ans[d] > maxx) return false;
  bool f = false;
  for (int i = max(last, b / a); i * a <= b * (dep + 1 - d); i++) {
    tem[d] = i;
    if (dfs(d + 1, a * i - b, b * i, i + 1)) {
      f = true;
    };
  }
  return f;
...
inline void work(signed CASE = 1, bool FINAL_CASE = false) {
  cin >> son >> mom;
  easify(mom, son);
  for (dep = 1; dep <= 50; dep++) {
    ans[dep] = INF;
    maxx = INF;
    if (dfs(1, son, mom, 1)) break;
  }
  for (int i = 1; i <= dep; i++) {
    cout << ans[i] << " ";
  }
