...
inline void work(signed CASE = 1, bool FINAL_CASE = false) {
  ...
  for (int i = n; i >= 2; i--) {
    int j = p[i], lf = l[j], rt = r[j];
    ll lv = abs(a[j].fi - a[lf].fi), rv = abs(a[rt].fi - a[j].fi);
    if (lv <= rv) {
      ans[i] = {lv, a[lf].second};
    } else {
      ans[i] = {rv, a[rt].second};
    }
    l[rt] = lf, r[lf] = rt;
  }
  for (int i = 2; i <= n; i++) {
    cout << ans[i].fi << " " << ans[i].second << endl;
  }
}
